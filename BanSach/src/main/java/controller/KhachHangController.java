...
public class KhachHangController extends HttpServlet {
			...
			if (baoLoi.length() > 0) {
				...
				if (khachHangDAO.insert(kh) > 0) {
					...
					if(khachHangDAO.updateVerifyInformation(kh)>0) {
						...
						Email.sendEmail(kh.getEmail(), "Xác thực tài khoản tại TITV.vn", getNoiDung(request, kh));
					}
				}
	...
	public static String getNoiDung(HttpServletRequest request, KhachHang kh) {
		String urlDefault = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()
		+ request.getContextPath();
		String link = urlDefault + "/khach-hang?hanhDong=xac-thuc&maKhachHang="+kh.getMaKhachHang()+"&maXacThuc="+kh.getMaXacThuc();
